...
export function formatEditorialReviewCardV2(state: EditorialDraftStateV2): string {
  ...
  return [
    ...
    `inquiry: ${state.draft.editorialCase?.inquiry?.question ?? "legacy — no editorial reasoning"}`,
    `why this evidence: ${state.draft.editorialCase?.inquiry?.whyThisEvidence ?? "legacy"}`,
    `check method: ${state.draft.editorialCase?.inquiry?.check ?? "legacy"}`,
    `learned / changed: ${state.draft.editorialCase?.inquiry?.memory ? JSON.stringify(state.draft.editorialCase.inquiry.memory) : "none — first observation"}`,
