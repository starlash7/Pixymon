...
const PIXYMON_CONCEPT_SIGNAL =
  /(픽시몬|pixymon|온체인\s*데이터를?\s*먹|영양소|소화|진화|레벨업|레벨\s*\d|사이클\s*먹|먹은\s*단서|먹은\s*걸로\s*치지\s*않|먹지\s*않|먹기엔|먹을\s*단서|채집한\s*단서|체인\s*로그를?\s*소화|주워\s*온\s*(?:단서|건)|건진\s*단서|덜\s*소화된\s*신호|내\s*장부|장부에\s*남긴다|장부에\s*넣지\s*않는다|장부에\s*올리지\s*않는다|진화에\s*쓸\s*단서|바로\s*먹을\s*만한|삼키지\s*않|삼키기엔|쉽게\s*삼켜지|한입에\s*설명|한\s*번\s*더\s*씹|천천히\s*소화|입에\s*(?:넣|걸린)|허기|사냥|냄새가\s*남|오래\s*남는\s*흔적|끝까지\s*남는\s*근거|반짝이는\s*서사보다|오늘\s*단서로\s*남긴|바로\s*믿기엔\s*이르|쉽게\s*믿지\s*않는다|서두르지\s*않는다|늦게\s*틀리는\s*편이\s*낫)/i;
