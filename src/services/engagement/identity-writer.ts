...
const FOCUS_ATTITUDE_BY_LANE: Partial<Record<TrendLane, Partial<Record<WriterFocus, string[]>>>> = {
  ecosystem: {
    retention: [
      ...
      "다시 들어오는 손이 식는 순간 큰 생태계 문장도 바로 광고처럼 얇아진다.",
      "열기는 남아도 재방문이 빠지는 날의 생태계는 오래 못 버틴다.",
      "다음 날 들어오는 사람이 줄어드는 순간 그 생태계 반응은 바로 캠페인 톤으로 기운다.",
      "재방문이 끊긴 열기는 크더라도 오래 못 가고 결국 포스터 값으로 남는다.",
    ],
    builder: [
  ...
  protocol: {
    durability: [
      ...
      "검증자 숫자가 남아도 운영 로그가 늦는 릴리스는 쉽게 체급을 못 얻는다.",
      "복구가 얇은데 기대감만 커지는 릴리스는 오래 들고 갈 이유가 없다.",
      "운영 흔적보다 쇼케이스 반응이 먼저 붙는 출시는 늘 반값으로 본다.",
      "장애 뒤 기록이 비는 개선은 박수보다 빈칸을 더 오래 남긴다.",
      "롤아웃 설명이 선명해도 복구 로그가 늦으면 그 개선은 아직 종이 위다.",
    ],
    launch: [
  ...
  "market-structure": {
    ...
    settlement: [
      ...
      "정산 깊이가 빠진 거래량은 커 보여도 결국 속도 자국만 남긴다.",
      "숫자는 선명해도 깊이가 못 눕는 장면은 끝내 구조값을 못 얻는다.",
    ],
  },
};
...
const FOCUS_EVIDENCE_BY_LANE: Partial<Record<TrendLane, Partial<Record<WriterFocus, string[]>>>> = {
  ...
  protocol: {
    ...
    launch: [
      ...
      "{A}가 버텨도 {B}가 비면 이 런치는 운영 문턱 앞에서 멈춘다.",
      "{A}가 선명해도 {B}가 안 따라오면 그 메인넷 얘기는 발표 체급을 못 벗어난다.",
      "{A}가 살아도 {B}가 늦으면 이 출시는 무대 밖 설득까지는 끝내 못 간다.",
      "{A}가 또렷해도 {B}가 머뭇거리면 그 런치 서사는 준비도보다 빈칸을 더 크게 남긴다.",
    ],
  },
  onchain: {
    flow: [
...
const FOCUS_DECISION_BY_LANE: Partial<Record<TrendLane, Partial<Record<WriterFocus, string[]>>>> = {
  ...
  protocol: {
    durability: [
      ...
      "복구 로그가 늦게 붙으면 이 발표는 릴리스보다 운영 빈칸 쪽으로 기운다.",
      "장애 뒤 흔적이 얇은 개선은 결국 박수보다 운영 공백을 더 크게 남긴다.",
      "운영 기록이 끝까지 못 붙으면 이 업그레이드는 설명보다 빈칸으로 기억된다.",
    ],
    launch: [
...
const MODE_STAMP_BY_LANE_AND_MODE: Partial<Record<TrendLane, Partial<Record<string, string[]>>>> = {
  ecosystem: {
    "era-manifesto": [
      ...
      "다음 국면의 문은 결국 남아 있는 사람 수가 열고 닫는다.",
      "열기만 남고 사람이 비는 생태계는 새 세대를 끝내 못 연다.",
      "생태계의 시대감은 결국 다시 돌아온 사람 수가 마지막에 정한다.",
    ],
    "identity-journal": [
      "남는 건 늘 열기보다 잔류 쪽이다.",
  ...
  protocol: {
    durability: {
      ...
      "meta-reflection": [
        ...
        "복구 로그가 늦는 날엔 발표보다 운영 빈칸이 먼저 이 릴리스의 허세를 벗긴다.",
        "장애 뒤 기록이 얕으면 좋은 릴리스도 결국 운영보다 발표회 톤으로 남는다.",
        "복구 태도가 비는 자리에서는 박수보다 로그 빈칸이 이 릴리스의 값을 다시 깎는다.",
      ],
    },
    launch: {
      "era-manifesto": [
...
const FOCUS_PRESSURE_BY_LANE: Partial<Record<TrendLane, Partial<Record<WriterFocus, string[]>>>> = {
  ecosystem: {
    ...
    retention: [
      ...
      "반응이 커도 다음 날 사람이 비는 장면은 결국 캠페인 값으로 밀린다.",
      "열기보다 생활 리듬이 먼저 끊기는 장면은 좋은 설명도 바로 값을 잃는다.",
      "사람 수가 먼저 얇아지는 장면은 대개 생태계 서사보다 포장 냄새를 더 크게 남긴다.",
    ],
    hype: [
...
function buildSceneFamilyFixationPool(input: KoIdentityWriterInput, focus: WriterFocus): string[] {
  ...
  if (input.lane === "ecosystem" && focus === "retention") {
    if (/retention\+usage|usage\+wallet/.test(sceneFamily)) {
      pool.push(
    ...
    if (/habit\+retention|return\+habit/.test(sceneFamily) || /생활|습관/.test(headline)) {
      pool.push(
  ...
  if (input.lane === "protocol" && focus === "durability") {
    ...
    if (/ops\+recovery|recovery\+validator|repair\+validator|repair\+ops|recovery\+ops/.test(sceneFamily)) {
      pool.push(
        ...
        "장애 뒤 기록이 밀리는 순간 그 개선은 운영 체급부터 얇아진다.",
        "복구 속도가 머뭇거리면 좋은 업그레이드도 끝내 체급을 못 얻는다.",
        "장애 뒤 태도가 늦게 눕는 순간 그 개선은 릴리스보다 시험대에 더 가까워진다.",
        "복구 흔적이 얇은 릴리스는 시간이 갈수록 발표보다 운영 빈칸을 더 크게 남긴다."
      );
    }
  }
  ...
  if (input.lane === "protocol" && focus === "durability") {
    if (/ops\+validator|validator\+log/.test(sceneFamily)) {
      pool.push(
        ...
        "검증자 숫자가 버텨도 운영 로그가 비면 이 릴리스는 결국 쇼케이스 톤으로 얇아진다.",
        "합의 숫자가 멀쩡해도 로그 빈칸이 남는 순간 이 개선은 발표값으로 다시 깎인다."
      );
    }
    if (/ops\+recovery|recovery\+validator|repair\+validator/.test(sceneFamily)) {
      pool.push(
        ...
        "복구 태도가 늦게 눕는 순간 이 릴리스는 개선보다 시험대 쪽으로 먼저 기운다.",
        "장애 뒤 기록이 밀린 자리에서는 좋은 업그레이드도 결국 운영보다 발표회를 닮는다."
      );
    }
  }
...
function buildSceneFamilyConsequencePool(input: KoIdentityWriterInput, focus: WriterFocus): string[] {
  ...
  if (input.lane === "protocol" && focus === "durability") {
    if (/ops\+validator|validator\+log|ops\+recovery|recovery\+validator|repair\+validator/.test(sceneFamily)) {
      pool.push(
  ...
  if (input.lane === "protocol" && focus === "launch") {
    if (/return\+ops|return\+announcement|return\+showcase|launch\+showcase|launch\+ops/.test(sceneFamily)) {
      pool.push(
  ...
  if (input.lane === "market-structure" && focus === "settlement") {
    if (/execution\+depth|fill\+depth|fill\+book|volume\+settlement|volume\+depth|depth\+settlement/.test(sceneFamily)) {
      pool.push(
