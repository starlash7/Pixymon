...
- `EditorialInquiryV2`: writer 앞의 편집 모델이 질문·근거의 중요성·현재 판단을 작성하고, 이전 판단/재관측 결과를 참조해 검사 방법을 선택한다. `pre-move-level`은 완전 되돌림, `current-level`은 현재 수준 유지 여부를 실제 falsifier로 검사한다. 관측만 남기거나 no-post할 수도 있다. 이전 검사 기준을 소급 변경하지 않는다.
...
가설 결과가 `digesting`인 것과 현재의 편집 판단은 별개다. 근거가 왜 중요한지까지 무조건 보류로 표현하지 않는다. 기억은 이전 글뿐 아니라 원래 질문·검사 방법·실제 체크포인트 결과를 포함하며, 신선도와 중복 게이트 이후 대상 선택에도 영향을 준다. 자유로운 조사 도구 실행이나 범용 자율성까지 구현한 것은 아니다.
